...
<h2 id="plugin">As a Claude Code plugin</h2>
<p>
  The skill and the hooks together install from inside Claude Code, without
  running <code>braids skill --install</code> or <code>braids hooks
  --install</code> yourself:
</p>
{sh("""
...
""")}
<p>
  It cannot install the binary. Do that first, or Claude is simply told braids
  is not here: the skill's first instruction is to check, so nothing guesses.
  The plugin's hooks run <code>braids hook</code> from your PATH rather than an
  absolute path, because a plugin cannot know where a binary it did not install
  ended up. With no braids on PATH those hooks do nothing, which was tested
  rather than assumed: a session with a hook whose command is missing runs
  normally and exits cleanly.
</p>
<p>
  If you have already run <code>braids skill --install</code>, installing the
  plugin gives you the same instructions twice, loaded as <code>braids</code>
  and <code>braids:braids</code>. Nothing in Claude Code says so, because from
  its side they are two skills that happen to agree, so braids says it instead:
  <code>braids skill</code> and <code>braids doctor</code> both report it and
  name the two ways out. Keep the plugin and run
  <code>braids skill --remove</code>, or keep the hand-installed one and
  uninstall the plugin.
</p>
{say("The plugin ships a copy of the skill, because a plugin is a directory and the binary embeds its own. Two copies of anything drift, so the copy is generated by <code>make plugin</code> and a test fails when somebody edits one and not the other. This repository has been bitten by that twice.")}
